...
def intensity(v, B=1, v_th=1, p=1):
    '''
    intensity function for spking model
    '''
    x = v - v_th 
...
def sim_glm_pop(J, E, tstop=100, dt=.01, B=1, v_th=1, p=1, tstim=None, Estim=0, v0=0, maxspikes = 2*10**6):
    '''
    simulate poisson spking generalized linear model
    
    J -- neuron-level conectivity
    E -- external input; either scalar of length J.shape[0]
    tstop -- stop time
    dt -- timestep 
    B -- gain
    v_th -- threshold for intensity function
    p -- exponent for intensity function
    tstim -- time to begin stimulation
    Estim -- total input during stimulation
    v0 -- initial condition
    maxspikes -- maximum number of spikes allowed (will stop simulation when this is exceeded)
