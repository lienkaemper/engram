...
def find_iso_rate_ca3(yca1, yca3, h, J0, g, g_ii, b, h_i_min, h_i_max,type, n_points = 200):
    ...
    returns:
    (h_i1, h_i3) -- inhibitory plasticity onto CA1 Engram, CA3 engram. Engram inhibition is multiplied, i.e. 
...
def find_iso_rate_input(target_rate_1, target_rate_3, J, b, b0_min = .1, b0_max = 5, p=2, n_points = 200, plot = False):
    ...
    returns:
    b_new -- input so that the engram rates match the target_rate 
    '''
    ...
    for i, b0 in enumerate(b0s):  #outer loop: CA3
        ...
        if p == 1:
            ...
            y_h =  y_0_quad(J,  b_new, steps = 500)[0]
            y_hs[i] = y_h
        if y_h >= target_rate_3:
            for i, b0 in enumerate(b0s):  #outer loop: CA3
                ...
                if p == 1:
                    ...
                    y_h =  y_0_quad(J,  b_new, steps = 500)[3]
                    y_hs[i] = y_h
...
def CA3_prop(J, r, b, nterms = None):
    ...
    J -- connectivity matrix (not already linearized)
    r -- rate
    b -- input
    '''
    gain =  2*(J@r+b)
...
def delta_hat(J, omega, r, b):
    '''Compute the propagator in the frequency domain.
    
    J -- neuron level connectivity matrix (not already linearized)
    omega -- freqnency
    r -- rate
    b -- input
    '''
    J_lin = J * (2 * (J @ r +b))[...,None]
    N = J.shape[0]
    return np.linalg.inv(np.eye(N) - (1/(1 +1j*omega)) *J_lin)
    
def sigma_hat(J, omega, r, b):
    '''Compute the covariance in the frequency domain.
    ...
    '''
    R = np.diag(r)  
    return delta_hat(J, omega, r, b) @ R @ delta_hat(J, -omega, r, b).T

def loop_by_integration(J, r, b, f_min = -25, f_max = 25, n_f = 2048):
    '''Compute the loop correction via frequency integration 
    ...
    f_min -- min frequency
    f_max -- max freqency
    n_f -- number of freqencies to integrate over
    '''

    N = J.shape[0]
    f = np.linspace(f_min, f_max, n_f)
    d_omega = (f_max - f_min)/n_f
    result = np.zeros((N, N), dtype= complex)

    for omega in f:
        corr= sigma_hat(J, omega, r, b)
        result += (1/(1 +1j*omega)) * (1/(1 +1j*(-omega))) *J @ corr @ J.T * d_omega / (2*np.pi)

    return np.real(delta_hat(J, 0, r, b) @ np.diag(result))