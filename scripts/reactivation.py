...
gc.collect()
v, spktimes = sim_glm_pop(J=J,  E=b, dt = dt, tstop=tstop/2, tstim = tstim/2,  Estim=b_stim, v_th = 0, maxspikes = maxspikes, p = 2)
raster_plot(spktimes, range(N),0, tstop/2, yticks = yticks,ax = axs[0,0], s_scale = 0.5)
...
trials =10
n_stim = 5
stims_sim = np.linspace(min_stim, max_stim, n_stim)
result_sim = np.zeros((6, n_stim))
for trial in range(trials):
    for i, stim in enumerate(stims_sim):
        b_stim= np.copy(b)
        b_stim[:int(N_E/2)] += stim
        v, spktimes = sim_glm_pop(J=J,  E=b_stim, dt = dt, tstop= tstop, v_th = 0, maxspikes = maxspikes, p = 2)
        rates = np.array([rate(spktimes, i, tstop) for i in range(N)])
        result_sim[:,i] += mean_by_region(rates, index_dict)/trials

axs[1,0].plot(stims, result[3, :].T, color = "#F37343", label = "Engram")
axs[1,0].plot(stims, result[4, :].T, color = "#06ABC8", label = "Non-engram")
axs[1,0].scatter(stims_sim, result_sim[3, :].T, color = "#F37343", label = "Engram")
axs[1,0].scatter(stims_sim, result_sim[4, :].T, color = "#06ABC8", label = "Non-engram")
...
J_small = sum_by_region(J, index_dict=index_dict)
b_iso =find_iso_rate_input(target_rate_1= y_baseline[3], target_rate_3= y_baseline[0], J = J_small, b = b_small, b0_min = 0, b0_max = .5, n_points=1000, plot = False)
...
gc.collect()
v, spktimes = sim_glm_pop(J=J,  E=b, dt = dt, tstop=tstop/2, tstim = tstim/2,  Estim=b_stim, v_th = 0, maxspikes = maxspikes, p = 2)
raster_plot(spktimes, range(N),0, tstop/2, yticks = yticks, ax = axs[0,1], s_scale= .5)
...
axs[1,1].scatter(stims_sim, result_sim[3, :].T, color = "#F37343", label = "Engram")
axs[1,1].scatter(stims_sim, result_sim[4, :].T, color = "#06ABC8", label = "Non-engram")
